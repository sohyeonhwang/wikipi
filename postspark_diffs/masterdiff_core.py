...
def parse_args():
    ...
    parser.add_argument('-ofn', '--output-filename', help='filename for the output file of tsv', default='monthly_diff_CORE', type=str)
...
def diff_find(row):
    r_current = row['core_regexes']
    r_prev = row['core_prev']
    # revision has a difference in regex from last revision of article
    r_bool = row['core_diff_bool']
    revert = row['revert']
...
if __name__ == "__main__":
    ...
    for pd_df in pd_dfs:
        ...
        pd_df.drop(['regexes','regexes_prev','regexes_diff_bool','reverteds'], axis=1, inplace=True)
        pd_df['regexes_diff'] = ""
